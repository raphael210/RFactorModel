...
#' @param factorFun a non-empty character string naming the function to get the factor scores.
...
getMultiFactor <- function(TS,FactorLists,wgts){  
  ...
  if(!missing(wgts)){
    if(length(FactorLists) != length(wgts)) {
      stop("The lengths of FactorLists and wgts are not equal!")
    }
    if(!isTRUE(all.equal(sum(wgts),1,tolerance=0.001))){
      warning("The sum of wgts is not 1. The wgts will be rescaled! ")
      wgts <- wgts/sum(wgts)
    }
    factorNames <- sapply(FactorLists,function(x)x$factorName)
...
getMultiFactorbyTSFs <- function(TSFs,wgts){
  ...
  if(!missing(wgts)){
    if(length(TSFs) != length(wgts)) {
      stop("The lengths of TSFs and wgts are not equal!")
    }
    if(!isTRUE(all.equal(sum(wgts),1,tolerance=0.001))){
      warning("The sum of wgts is not 1. The wgts will be rescaled! ")
      wgts <- wgts/sum(wgts)
